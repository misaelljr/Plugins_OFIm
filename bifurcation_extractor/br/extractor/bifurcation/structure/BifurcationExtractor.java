...
import extractor.bifurcation.nifti_reader.NiftiVolume;
import br.oracle.pluginInterfaces.InterfaceImageExtractor;
import br.oracle.pluginInterfaces.OFImPlugin;

public class BifurcationExtractor extends OFImPlugin implements InterfaceImageExtractor{
	
	private final String name = "BifurcationExtractor";
	String [] file;
	Bifurcation bifurc;
	...
	public double computeValue(Object complexObject) throws IOException {
		...
		if (file.length == 1){	
			quant_bifurc = bifurc.extractorCountBifurc(volume.data, nx, ny, nz, dim);
		}else{
			volume.write(file[1]);
		}	
		return quant_bifurc;
